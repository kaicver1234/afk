...
function controls(n){open('کنترل '+esc(n),'<div class="controls"><button onclick="act(\''+n+'\',\'control\',{control:\'forward\',state:true})">جلو</button><button onclick="act(\''+n+'\',\'control\',{control:\'back\',state:true})">عقب</button><button onclick="act(\''+n+'\',\'control\',{control:\'left\',state:true})">چپ</button><button onclick="act(\''+n+'\',\'control\',{control:\'right\',state:true})">راست</button><button onclick="act(\''+n+'\',\'control\',{control:\'jump\',state:true})">پرش</button><button onclick="act(\''+n+'\',\'control\',{control:\'sneak\',state:true})">نشست</button><button onclick="act(\''+n+'\',\'control\',{control:\'sprint\',state:true})">دویدن</button><button onclick="act(\''+n+'\',\'stopControls\')">توقف حرکت</button></div><div class="form"><input id="chat" placeholder="پیام یا command"><button onclick="sendChat(\''+n+'\')">ارسال</button></div><div class="form"><input id="yaw" placeholder="yaw رادیان"><input id="pitch" placeholder="pitch رادیان"><button onclick="look(\''+n+'\')">چرخش دوربین</button></div>')}
async function sendChat(n){const v=el('chat').value;if(v)act(n,'chat',{text:v})}async function look(n){act(n,'look',{yaw:Number(el('yaw').value||0),pitch:Number(el('pitch').value||0)})}
...
async function routeApi(req, res, url, o) {
  ...
  if (parts.length === 2 && parts[1] === 'bots' && req.method === 'GET') return json(res, 200, o.list().map(snapshot));
  if (parts.length === 2 && parts[1] === 'bots' && req.method === 'POST') {
    ...
    if (!/^[a-zA-Z0-9.-]{1,253}$/.test(String(b.host || ''))) throw Error('آدرس سرور نامعتبر است');
    await o.connect({ name: b.name, host: b.host, port, version: b.version || 'auto' }); return json(res, 201, { ok: true });
  }
